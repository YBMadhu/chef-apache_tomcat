...
describe 'apache_tomcat::default' do
  let(:chef_run) do
    ChefSpec::SoloRunner.new(step_into: ['apache_tomcat'],
      ...
      node.set['apache_tomcat']['instance_path'] = '/foo'
  ...
  it 'includes the _instance recipe' do
    expect(chef_run).to include_recipe('apache_tomcat::_instance')
  end
  ...
  it 'creates tomcat instance' do
    expect(chef_run).to create_apache_tomcat_instance('/foo')
  end
